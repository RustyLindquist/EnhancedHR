...
import { BookOpen, Clock, Award, LayoutGrid, List, ChevronRight, Plus } from 'lucide-react';
import AddCollectionModal from '@/components/AddCollectionModal';
import { Collection } from '@/types';
import { fetchUserCollections, createCollection } from '@/lib/collections';
import { syncCourseCollectionsAction } from '@/app/actions/collections';
...
export default function ExpertCoursesContent({ courses, expertFirstName, userId }: ExpertCoursesContentProps) {
    const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
    const [collections, setCollections] = useState<Collection[]>([]);
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [modalCourse, setModalCourse] = useState<CourseData | null>(null);
    ...
    // Fetch user collections when logged in
    useEffect(() => {
        if (userId) {
            fetchUserCollections(userId).then(setCollections);
        }
    }, [userId]);
    ...
    const handleAddClick = (e: React.MouseEvent, course: CourseData) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    const handleSaveToCollection = async (selectedCollectionIds: string[], newCollection?: { label: string; color: string }) => {
        if (!userId || !modalCourse) return;

        let finalSelectionIds = [...selectedCollectionIds];

        // Handle new collection creation
        if (newCollection) {
            const created = await createCollection(userId, newCollection.label, newCollection.color);
            if (created) {
        ...
        // Sync course to collections
        const targetIds = finalSelectionIds.filter(id => id !== 'new');
        await syncCourseCollectionsAction(userId, modalCourse.id, targetIds);

        // Dispatch refresh events so other parts of the app update
        window.dispatchEvent(new Event('collection:refresh'));
        window.dispatchEvent(new Event('dashboard:invalidate'));
    };

    const renderAddButton = (course: CourseData) => {
        if (!userId) return null;
        return (
            <button
                onClick={(e) => handleAddClick(e, course)}
                className="absolute top-3 right-3 z-10 p-2 rounded-full bg-black/60 backdrop-blur-sm text-white/60 hover:text-white hover:bg-black/80 transition-all opacity-0 group-hover:opacity-100"
    ...
    const renderListAddButton = (course: CourseData) => {
        if (!userId) return null;
        return (
            <button
                onClick={(e) => handleAddClick(e, course)}
    ...
    return (
        <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {courses.map(course => (
                                <Link
                                    ...
                                    className="group relative rounded-2xl overflow-hidden bg-white/5 border border-white/10 hover:border-brand-blue-light/30 transition-all hover:-translate-y-1"
                                >
                                    {/* Course Image */}
            ...
            {isAddModalOpen && modalCourse && (
                <AddCollectionModal
                    item={{ type: 'COURSE', id: modalCourse.id, title: modalCourse.title } as any}
                    availableCollections={collections}
                    onClose={() => { setIsAddModalOpen(false); setModalCourse(null); }}
