...
import { signupExpert, verifyExpertEmail, resendExpertVerification } from './actions'
...
export default function ExpertSignupPage() {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (view === 'signup') {
            const result = await signupExpert(formData)
            ...
            if (result?.error) {
                ...
                if (result.view === 'verify' && result.email) {
                    setView('verify')
            ...
            const result = await verifyExpertEmail(formData)
    ...
    const handleResendCode = async () => {
        ...
        formData.append('email', verificationEmail)
        const result = await resendExpertVerification(formData)
        ...
        <div className="min-h-screen w-full bg-[#051114] flex flex-col relative overflow-hidden font-sans text-white">
            ...
            <header className="absolute top-0 left-0 w-full p-8 z-20 flex justify-between items-center">
                <div className="flex flex-col">
                    ...
                    <h1 className="text-2xl font-light tracking-wide text-white">EXPERT ACCESS</h1>
                </div>
                    ...
                    <div className="bg-[#0f172a]/80 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
                        ...
                        <div className="mb-8 text-center">
                            <h2 className="text-xl font-bold text-white mb-2">
                                {view === 'signup' ? 'Become an Expert' : 'Verify Your Email'}
                            </h2>
                            <p className="text-slate-400 text-sm">
                                {view === 'signup'
                                    ? 'Create your free expert account to submit a course proposal.'
                        ...
                        <form onSubmit={handleSubmit} className="space-y-5">
                            ...
                            >
                                {isLoading ? (
                                    ...
                                    <>
                                        {view === 'signup' ? 'Create Expert Account' : 'Verify Account'} <ArrowRight size={18} />
                                    </>
                                )}
                            </button>
                        </form>
