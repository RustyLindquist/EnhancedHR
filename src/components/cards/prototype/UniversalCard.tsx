...
import { Trash2, Plus, Play, FileText, MessageSquare, Clock, Download, Edit, Paperclip, Star, Award } from 'lucide-react';

export type CardType = 'COURSE' | 'MODULE' | 'LESSON' | 'RESOURCE' | 'CONVERSATION' | 'CONTEXT';
...
interface UniversalCardProps {
    ...
    rating?: number; // 0-5
    credits?: { shrm?: number; hrci?: number }; // Available credits
    onAction?: () => void;
}
...
const UniversalCard: React.FC<UniversalCardProps> = ({
    ...
    const config = {
        ...
        CONVERSATION: { // Blue/Teal
            headerColor: 'bg-[#1e293b]', // Slate-ish
        ...
        CONTEXT: { // Gold/Brown
            headerColor: 'bg-[#3D2C10]',
            borderColor: 'border-yellow-600/30',
            labelColor: 'text-yellow-200',
            barColor: 'bg-yellow-900/40',
            icon: FileText,
            buttonStyle: 'bg-white/10 hover:bg-white/20 text-white'
        }
    }[type];
    ...
    const isTextHeavy = ['CONVERSATION', 'CONTEXT', 'RESOURCE'].includes(type);
    const topHeight = isTextHeavy ? 'h-[40%]' : 'h-[60%]';
    const bottomHeight = isTextHeavy ? 'h-[60%]' : 'h-[40%]';

    return (
        <div className={`relative group w-full aspect-[4/3] rounded-3xl overflow-hidden border border-white/10 bg-[#0B1120] shadow-2xl transition-all hover:scale-[1.02]`}>
            ...
            <div className={`relative ${topHeight} w-full overflow-hidden ${isMediaCard ? 'bg-black' : config.barColor} transition-all duration-300`}>
                ...
                {isMediaCard && imageUrl && (
                    <>
                        <div className="absolute inset-0 bg-cover bg-center opacity-60" style={{ backgroundImage: `url(${imageUrl})` }}></div>
                ...
                <div className="absolute top-0 left-0 w-full p-3 z-20 flex flex-col gap-2">
                    ...
                    <div className="flex items-center justify-between bg-black/40 backdrop-blur-sm px-3 py-1.5 rounded-lg border border-white/5 shadow-sm">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                            <button className="text-white/40 hover:text-white transition-colors p-1"><Trash2 size={14} /></button>
                            <button className="text-white/40 hover:text-white transition-colors p-1"><Plus size={16} /></button>
                        </div>
                    </div>
