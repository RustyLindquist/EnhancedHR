...
import { ChevronDown, Check, Flame, Users } from 'lucide-react';
import { fetchPlatformActivityData, PlatformActivityData } from '@/app/actions/platform-activity';
import ActivityHeatmap from '../Dashboard/charts/ActivityHeatmap';
import PersonalActivityTrendsChart, { CATEGORY_CONFIG, ALL_CATEGORIES } from '../Dashboard/charts/PersonalActivityTrendsChart';
...
const STAT_CARDS = [
    ...
    { key: 'activeUsers', label: 'Active Users', color: 'text-orange-400' },
] as const;
...
const CACHE_KEY = 'ehr:platform-activity';
...
const PlatformActivityWidget: React.FC = () => {
    ...
    const [activityData, setActivityData] = useState<PlatformActivityData | null>(null);
    ...
    useEffect(() => {
        ...
        const doFetch = () => {
            if (!hasLoadedRef.current) {
                const cacheKey = `${CACHE_KEY}:${startDate.toISOString().split('T')[0]}:${endDate.toISOString().split('T')[0]}`;
                const cached = readCache<PlatformActivityData>(cacheKey);
            ...
            fetchPlatformActivityData({ startDate: startStr, endDate: endStr, includeHeatmap: false })
                .then(data => {
                    if (version !== requestVersionRef.current) return;
                    hasLoadedRef.current = true;
                    setActivityData(data);
                    setLoading(false);
                    const cacheKey = `${CACHE_KEY}:${startStr}:${endStr}`;
                    writeCache(cacheKey, data);
                })
                .catch(err => {
                    if (version !== requestVersionRef.current) return;
                    console.error('Failed to fetch platform activity data:', err);
                    setLoading(false);
                });
        };
    ...
    // Derive heatmap data from daily activity
    ...
    const animActiveUsers = useCountUp(stats?.activeUsers || 0);
    ...
    const getStatValue = (key: string): string => {
        switch (key) {
            ...
            case 'activeUsers': return animActiveUsers.toString();
            default: return '0';
        }
    };
...
export default PlatformActivityWidget;
