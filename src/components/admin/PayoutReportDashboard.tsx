...
export default function PayoutReportDashboard({
    ...
    const exportToCSV = () => {
        const headers = ['Expert Name', 'Email', 'Watch Time (min)', 'Citations', 'Watch Earnings', 'Citation Earnings', 'Total Payout'];
        const rows = payoutData.map(author => [
            author.authorName,
            author.email,
            author.totalWatchTime.toFixed(2),
            author.totalCitations.toString(),
            author.watchEarnings.toFixed(2),
            author.citationEarnings.toFixed(2),
            author.totalPayout.toFixed(2)
        ]);
        ...
        rows.push([
            ...
            grandTotals.citations.toString(),
            grandTotals.watchEarnings.toFixed(2),
            grandTotals.citationEarnings.toFixed(2),
            grandTotals.totalPayout.toFixed(2)
        ]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
                ...
                <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-5">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <span className="text-2xl font-bold text-yellow-400">{formatCurrency(grandTotals.watchEarnings + grandTotals.citationEarnings)}</span>
                    </div>
                    <p className="text-sm text-slate-400">Combined Earnings</p>
                </div>

                <div className="bg-gradient-to-r from-brand-blue-light/10 to-purple-500/10 border border-white/20 rounded-xl p-5">
                    <div className="flex items-center justify-between mb-2">
                        <DollarSign className="text-white" size={20} />
                        <span className="text-2xl font-bold text-white">{formatCurrency(grandTotals.totalPayout)}</span>
                    </div>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between bg-white/5 border border-white/10 rounded-xl p-4">
                ...
                <button
                    onClick={exportToCSV}
                    className="flex items-center gap-2 px-4 py-2 bg-brand-blue-light text-brand-black font-bold rounded-lg hover:bg-white transition-colors"
                >
                    <Download size={16} />
                    Export CSV
                </button>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center gap-4">
                ...
                <div className="text-sm text-slate-400">
                    <strong className="text-white">Current Payout Rates:</strong>{' '}
                    <span className="text-green-400">$0.10 per minute watched</span> +{' '}
                    <span className="text-purple-400">$0.50 per AI citation</span>
                </div>
            </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden">
                ...
                <div className="grid grid-cols-12 gap-4 p-4 bg-white/5 border-b border-white/10 text-xs font-bold text-slate-500 uppercase tracking-wider">
                    ...
                    <div className="col-span-2 text-right">Watch Earnings</div>
                    <div className="col-span-2 text-right">Citation Earnings</div>
                    <div className="col-span-2 text-right">Total Payout</div>
                </div>
                ...
                {payoutData.length === 0 ? (
                    ...
                    payoutData.map((author, index) => {
                        const isExpanded = expandedAuthor === author.authorId;
                        return (
                            <div key={author.authorId}>
                                {/* Author Row */}
                                <div
                                    className={`grid grid-cols-12 gap-4 p-4 items-center cursor-pointer hover:bg-white/5 transition-colors ${index !== payoutData.length - 1 ? 'border-b border-white/5' : ''
                                        }`}
                                ...
                                >
                                    ...
                                    <div className="col-span-2 text-right text-green-400 font-medium">
                                        {formatCurrency(author.watchEarnings)}
                                    </div>
                                    <div className="col-span-2 text-right text-purple-400 font-medium">
                                        {formatCurrency(author.citationEarnings)}
                                    </div>
                                        ...
                                        <div className="space-y-2">
                                            {author.courses.map(course => (
                                                <div key={course.courseId} className="grid grid-cols-12 gap-4 py-2 text-sm border-b border-white/5 last:border-0">
                                                    <div className="col-span-5 text-slate-300 truncate pl-11">
                                                        {course.courseTitle}
                                                    </div>
                                                    ...
                                                    <div className="col-span-2 text-right text-green-400/70">
                                                        {formatCurrency(course.watchTimeMinutes * 0.10)}
                                                    </div>
                                                    <div className="col-span-2 text-right text-purple-400/70">
                                                        {formatCurrency(course.citations * 0.50)}
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                ...
                {/* Totals Row */}
                {payoutData.length > 0 && (
                    <div className="grid grid-cols-12 gap-4 p-4 bg-gradient-to-r from-brand-blue-light/10 to-purple-500/10 border-t border-white/20 items-center">
                        ...
                        <div className="col-span-2 text-right text-green-400 font-bold">
                            {formatCurrency(grandTotals.watchEarnings)}
                        </div>
                        <div className="col-span-2 text-right text-purple-400 font-bold">
                            {formatCurrency(grandTotals.citationEarnings)}
                        </div>
                        <div className="col-span-2 text-right text-xl text-white font-bold">
                            {formatCurrency(grandTotals.totalPayout)}
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-xl p-5">
                ...
                <ol className="text-sm text-slate-400 space-y-2 list-decimal list-inside">
                    ...
                    <li>Review each expert's earnings in the spreadsheet</li>
