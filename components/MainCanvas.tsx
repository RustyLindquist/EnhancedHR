...
import { Search, SlidersHorizontal, X, Check, ChevronDown, RefreshCw, Plus, ChevronRight, GraduationCap, Layers, Flame, MessageSquare, Sparkles } from 'lucide-react';
import CardStack from './CardStack';
import CollectionSurface from './CollectionSurface';
import AlertBox from './AlertBox';
import AddCollectionModal from './AddCollectionModal';
import { COURSE_CATEGORIES, DEFAULT_COLLECTIONS, COLLECTION_NAV_ITEMS } from '../constants';
...
const CustomDragLayer: React.FC<{ course: Course | undefined; x: number; y: number }> = ({ course, x, y }) => {
    if (!course) return null;
    return (
        <div 
            className="fixed pointer-events-none z-50 transform -translate-x-1/2 -translate-y-1/2 w-80 h-[28rem] opacity-90 scale-90"
            style={{ left: x, top: y }}
        >
            <div className="w-full h-full bg-slate-800/80 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl overflow-hidden flex flex-col">
                 <div className="h-44 w-full relative">
                    <img src={course.image} className="w-full h-full object-cover opacity-80" alt="" />
                 </div>
                 <div className="p-6">
                    <h3 className="text-xl font-bold text-white mb-2">{course.title}</h3>
                    <p className="text-sm text-slate-300">{course.author}</p>
                 </div>
            </div>
        </div>
    );
...
const MainCanvas: React.FC<MainCanvasProps> = ({ 
  ...
  const handleSaveToCollection = (selectedCollectionIds: string[], newCollection?: { label: string; color: string }) => {
    ...
    if (selectedCourseForModal) {
       const updatedCourses = courses.map(c => {
           if (c.id === selectedCourseForModal.id) {
               return {
                   ...c,
       ...
       const defaultIds = ['favorites', 'research', 'to_learn'];
       const addedDefault = selectedCollectionIds.find(id => defaultIds.includes(id));
       
       if (addedDefault) {
           setFlaringPortalId(addedDefault);
           setTimeout(() => setFlaringPortalId(null), 1000);
       }
    }
  ...
  const activeFilterCount = 
    ...
    (activeFilters.credits.length) + 
    (activeFilters.designations.length) + 
    (activeFilters.status.length) + 
    (activeFilters.ratingFilter !== 'ALL' ? 1 : 0) +
    (activeFilters.searchQuery ? 1 : 0) +
    (activeFilters.dateFilterType !== 'ALL' ? 1 : 0);
  ...
  return (
    ...
    >
        {isDragging && draggedCourseId && (
            <CustomDragLayer 
                course={courses.find(c => c.id === draggedCourseId)}
                x={mousePos.x}
                y={mousePos.y}
            />
        )}
          ...
          <div className="bg-[#0f172a]/95 backdrop-blur-2xl border-b border-white/10 shadow-2xl pb-6">
             <div className="max-w-7xl mx-auto px-10 pt-8">
                ...
                <div className="flex justify-between items-center mb-6">
                   <h2 className="text-white text-lg font-light tracking-widest uppercase">Search & Filter</h2>
                   <button 
                      ...
                      <div className="space-y-2 h-40 overflow-y-auto custom-scrollbar pr-2">
                         {COURSE_CATEGORIES.map(cat => (
                             <label key={cat} className="flex items-center space-x-3 cursor-pointer group">
                                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all flex-shrink-0 ${pendingFilters.categories.includes(cat) ? 'bg-brand-blue-light border-brand-blue-light' : 'border-slate-600 group-hover:border-slate-400'}`}>
                                   {pendingFilters.categories.includes(cat) && <Check size={12} className="text-black" />}
                                </div>
                                <span className={`text-sm truncate transition-colors ${pendingFilters.categories.includes(cat) ? 'text-white' : 'text-slate-400 group-hover:text-slate-200'}`}>{cat}</span>
                                <input type="checkbox" className="hidden" checked={pendingFilters.categories.includes(cat)} onChange={() => toggleArrayFilter('categories', cat)} />
                             </label>
                         ))}
                      </div>
                   </div>
                        ...
                        visibleCourses.map((course, index) => {
                         ...
                         activeCollectionId === 'conversations' ? (
                            ...
                            <div className="col-span-full flex flex-col items-center justify-center py-16 px-4">
                                {/* Text Content */}
                                <div className="max-w-4xl text-center animate-float mb-8">
                                    <h2 className="text-2xl font-light text-white mb-6">You haven’t had any conversations with Prometheus AI yet.</h2>
                                    <p className="text-slate-400 text-lg mb-10">Prometheus AI is your learning assistant. You can access Prometheus from several places.</p>
                                ...
                                {/* Visuals - Chat Interface Simulation */}
                                <div className="flex justify-center gap-6 opacity-40 select-none pointer-events-none mt-12 scale-90">
                                    {/* Card 1 - Left - Tilted */}
                            ...
                            <div className="col-span-full flex flex-col items-center justify-center py-16 px-4">
                                {/* Text Content */}
                                <div className="max-w-2xl text-center mb-12 animate-float">
                                    <h2 className="text-2xl font-light text-white mb-6">No content has been saved to this collection.</h2>
                                    <div className="text-slate-400 space-y-4 leading-relaxed font-light">
                                ...
                                {/* Visuals - 3 Ghost Cards */}
                                <div className="flex justify-center gap-6 opacity-30 select-none pointer-events-none">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className={`
        ...
        <div className="absolute bottom-0 left-0 w-full z-[60] pointer-events-none">
           <CollectionSurface 
                ...
                onDropCourse={(portalId) => {
                    if (draggedCourseId) {
                        if (portalId === 'new') {
                             handleAddButtonClick(draggedCourseId);
                        } else {
                             const course = courses.find(c => c.id === draggedCourseId);
                             if (course) {
                                 const currentCollections = course.collections || [];
                                 if (!currentCollections.includes(portalId)) {
                                     handleSaveToCollection([...currentCollections, portalId]);
                                 } else {
