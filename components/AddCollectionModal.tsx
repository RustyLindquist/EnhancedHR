...
import { Collection, Course } from '../types';
...
const AddCollectionModal: React.FC<AddCollectionModalProps> = ({ 
  ...
  const [selectedIds, setSelectedIds] = useState<string[]>(course?.collections || []);
  // If no course is provided, default to "Creating New" mode, otherwise "Adding" mode
  const [isCreatingNew, setIsCreatingNew] = useState(!course);
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-[#0f172a] border border-white/10 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden animate-float">
        ...
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div>
              <h2 className="text-xl font-bold text-white tracking-tight">
                {course ? 'Add to Collection' : 'Manage Collections'}
              </h2>
              {course && <p className="text-xs text-slate-400 mt-1 truncate max-w-[250px]">{course.title}</p>}
          </div>
          <button 
            onClick={onClose}
        ...
        <div className="px-6 pt-6 pb-0 max-h-[60vh] overflow-y-auto custom-scrollbar">
           ...
           {/* Existing Collections List - Only show if we have a course or we are not in create-only mode */}
           {course && (
               ...
               {!isCreatingNew ? (
                   ...
                   <div className="animate-fade-in">
                       <div className="flex justify-between items-center mb-4">
                           ...
                           {/* Only allow cancel if we are in "Add Course" mode where there's something else to do */}
                           {course && (
                               <button 
        ...
        <div className="p-6 border-t border-white/10 bg-black/20 flex justify-end gap-3 z-10 relative">
            ...
            >
                {isCreatingNew && !course ? 'Create Collection' : 'Save Changes'}
            </button>
        </div>
