-- Separate insight instructions into their own column for easy editing
-- This makes the insight training appendix editable in Admin Console

-- Step 1: Add the insight_instructions column
ALTER TABLE ai_system_prompts
ADD COLUMN IF NOT EXISTS insight_instructions TEXT DEFAULT '';

-- Step 2: Extract insight instructions from existing prompts and reset base prompts
-- We'll populate the new column with the enhanced instructions and reset base prompts

-- Platform Assistant - Extract insight instructions and reset base prompt
...
- [goal] + [skill] → Build on what they know toward where they want to go',
    system_instruction = 'You are the Platform Assistant for EnhancedHR. Your goal is to help users navigate the platform, find courses, and answer general HR questions. You have access to the entire library of courses and the user''s profile.'
...
Avoid: "I remember you told me..." or "According to what I know about you..."',
    system_instruction = 'You are the Course Tutor. Your goal is not just to answer, but to teach. Use Socratic methods, ask probing questions, and help the user apply the course concepts to their specific role and company context.'
...
**Important:** Never force references - only when genuinely relevant to their question about the course material.',
    system_instruction = 'You are the Course Assistant. You are an expert on this specific course. Answer questions based ONLY on the course material provided in the transcript and resources.'
...
Reference naturally without calling out that you''re using stored information.',
    system_instruction = 'You are the Collection Assistant. You are context-aware of the specific collection the user is viewing. Help them synthesize information across the courses in this collection.'
WHERE agent_type = 'collection_assistant';

-- Add comment explaining the column
COMMENT ON COLUMN ai_system_prompts.insight_instructions IS 'Separate insight training instructions appended to system_instruction at runtime. Editable via Admin Console for easy testing and refinement.';
